...
h2. <a name="1.2.14">Updates in 1.2.14</a>

* Bug fixed:
** "Model.cloud() function error":https://github.com/greenlaw110/play-morphia/issues/105
** "Group by field":https://github.com/greenlaw110/play-morphia/issues/102
** "Model.groupAverage doesn't respect Model.Column annotationhttps"://github.com/greenlaw110/play-morphia/issues/98
** "Application.conf morphia.autoTimestamp doesn't work at all":https://github.com/greenlaw110/play-morphia/issues/97
** "@Unique validation not working correctly when updating record":https://github.com/greenlaw110/play-morphia/issues/96

h2. <a name="1.2.13b">Updates in 1.2.13b</a>

* "Cache entity feature":cache
* <code>@OnUpdate</code> and <code>@Updated</code> annotated functions get triggered when calling @Model._update()@ interface
...
h2. <a name="1.5.0">Updates in 1.5.0</a>

* Update morphia library from `com.google.code.morphia 1.0` to `org.mongodb.morphia 0.107`. This is major change in the dependencies and it impact user application. Please do a text replace from "com.google.code.morphia" to "org.mongodb.morphia" in your application. In addition to morphia library update, the mongo-java-driver library has been updated to 2.12.2

h2. <a name="1.2.9">Updates in 1.2.9</a>

* Enhancements:
** Allow <code>@org.mongodb.morphia.annotations.Converters</code> annotation be put on field type class declaration
