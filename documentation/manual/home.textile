...
p(note). *Break changes*: Start from v1.2.8-RC1 "Auto timestamp feature enabled to all Model classes by default":model#timestamp. Add the following line to your @conf/application.conf@ file to maintain the compatibility to your existing project: @morphia.autoTimestamp=false@
